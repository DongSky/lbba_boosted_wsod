...
def proposal_target_layer_fast(rpn_rois, rpn_scores, gt_boxes, _num_classes):
    """
...
def _get_bbox_regression_labels(bbox_target_data, num_classes):
  ...
  """
    ...
    # bbox_targets = clss.new_zeros(clss.numel(), 84)
    ...
    if inds.numel() > 0:
        ...
        # print(bbox_target_data[inds].shape)
    ...
    # clss = bbox_target_data[:, 0]
    # bbox_targets = clss.new(clss.numel(), 4).zero_()
    ...
    #     print('net', bbox_target_data.shape)
    #     # print(bbox_targets.shape)
    #     # print(bbox_target_data[inds].shape)
    #     bbox_inside_weights[inds, :] = bbox_targets.new(cfg.TRAIN.BBOX_INSIDE_WEIGHTS).view(-1, 4).expand_as(dim1_inds)
...
def _compute_targets(ex_rois, gt_rois, labels):
    ...
    # ex_rois = ex_rois.to(torch.float32)
    # gt_rois = gt_rois.to(torch.float32)
