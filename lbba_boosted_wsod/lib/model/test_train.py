...
def apply_nms(all_boxes, thresh):
    ...
    for cls_ind in range(num_classes):
        for im_ind in range(num_images):
            ...
            keep = nms(torch.from_numpy(dets[:, :4]), torch.from_numpy(scores), thresh).numpy()
            if len(keep) == 0:
                continue
...
def test_train_net(net, imdb, weights_filename, max_per_image=100, thresh=0.):
    ...
    with open('results_12_trainval.csv','r') as f:
    ...
    for i in range(0):
        ...
        for j in range(1, imdb.num_classes + 1):
            inds = np.argmax(scores[:, j-1])
            all_boxes_fast[j][i] = \
                    np.hstack((boxes_fast[inds, :].reshape(1, -1),
                           np.array([[scores_fast[inds, j]]])))
            if im_pseudo_label[j-1]<-5:
                all_boxes_fast[j][i][:,-1] = 0.
        '''
            end of faster part
        '''
    ...
    #with open('864_12_trainval', 'wb') as f:
    #    pickle.dump(all_boxes_fast, f, pickle.HIGHEST_PROTOCOL)

    with open('merge_12_trainval.pkl', 'rb') as f:
        all_boxes_fast = pickle.load(f)  
    for i in range(num_images):
        for j in range(20):
            if len(all_boxes_fast[j][i])>0:
                cls_scores = all_boxes_fast[j][i][:, -1]
                cls_boxes = all_boxes_fast[j][i][:, :-1]
                inds = [np.argmax(cls_scores)]
                #keep = nms(torch.from_numpy(cls_boxes).float(), torch.from_numpy(cls_scores),cfg.TEST.NMS).numpy()
        
                all_boxes_fast[j][i] = all_boxes_fast[j][i][inds,:]
