...
ind_to_class = dict(list(zip(list(range(20)),classes)))
...
def save_xml(pseudo,i,imdb,im):
    ...
    for j in range(20):
        if pseudo[j]!=[]:
            print(pseudo[j])
            boxes = np.around(pseudo[j][:,:-1]).astype(np.int16)
    ...
    tree.write('data/VOCdevkit2007/VOC2007/Annotations_pseudo/{}.xml'.format(file_name), encoding='utf-8')
...
def generate_pseudo(net, imdb, weights_filename, max_per_image=100, thresh=0.):
    ...
    for i in range(num_images):
        ...
        im_real_label = imdb._load_pascal_labels(imdb._image_index[i])['labels']
        ...
        for j in range(1, imdb.num_classes + 1):
            ...
            # print(cls_dets)
            pseudo.append(cls_dets)
            # all_boxes_fast[j][i] = cls_dets
        # exit()
        save_xml(pseudo,i,imdb,im)
        # continue
