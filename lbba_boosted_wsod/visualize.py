...
#       keep = nms(torch.from_numpy(cls_boxes.astype(np.float32)), torch.from_numpy(cls_scores.astype(np.float32)), cfg.TEST.NMS).numpy() if cls_dets.size > 0 else []
...
if __name__ == "__main__":
    ...
    num_images = len(imdb.image_index)
    output_dir_fast = 'output/vgg16/voc_2007_test/default/vgg16_faster_rcnn_iter_195000' + '/' + 'faster'
    ...
    all_boxes_fast = pickle.load(open(det_file, 'rb'))
    print("Visualization Faster-RCNN Branch Results")
    for i in range(num_images):
        ...
        for cls_ind, cls in enumerate(CLASSES[1:]):
            ...
            # keep = nms(torch.from_numpy(dets[:, :4]), torch.from_numpy(dets[:, 4]), NMS_THRESH)
            # dets = dets[keep.numpy(), :]
            dets = all_boxes_fast[cls_ind][i][:]
            im = vis_detections(im, cls, dets, thresh=CONF_THRESH)
        cv2.imwrite("vis_result/" + "det_" + imname, im)
